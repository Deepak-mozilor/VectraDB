...
impl CodeChunker {
    ...
    fn chunk_by_structures(&self, content: &str, config: &ChunkingConfig, language: &str) -> Vec<Chunk> {
            ...
            if is_function_start || is_class_start {
                ...
                if current_chunk.len() > config.max_chunk_size {
                    chunks.push(Chunk {
                        ...
                        end_index: current_chunk.len(),
                        chunk_type: ChunkType::Code,
                        metadata: self.create_code_metadata(&current_chunk, start_line, language, "large"),
                    });
                ...
                if (in_function || in_class) && brace_count == 0 {
                    ...
                    chunks.push(Chunk {
                        ...
                        end_index: current_chunk.len(),
                        chunk_type: ChunkType::Code,
                        metadata: self.create_code_metadata(&current_chunk, start_line, language, structure_type),
                    });
                    current_chunk = String::new();
                    in_function = false;
                    in_class = false;
    ...
    fn chunk_by_logical_blocks(&self, content: &str, config: &ChunkingConfig, language: &str) -> Vec<Chunk> {
        let mut chunks = Vec::new();
        let lines: Vec<&str> = content.lines().collect();
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            let block_type = if trimmed.starts_with("import ") || trimmed.starts_with("use ") || trimmed.starts_with("from ") {
                "imports"
            ...
            if block_type != current_block_type && !current_block.is_empty() {
                ...
                chunks.push(Chunk {
                    ...
                    end_index: i,
                    chunk_type: ChunkType::Code,
                    metadata: self.create_code_metadata(&current_block.join("\n"), start_line, language, current_block_type),
                });
                current_block.clear();
                start_line = i;
            }
            ...
            if current_block.join("\n").len() > config.max_chunk_size {
                    ...
                    chunks.push(Chunk {
                        ...
                        end_index: i,
                        chunk_type: ChunkType::Code,
                        metadata: self.create_code_metadata(&chunk_content, start_line, language, current_block_type),
                    });
                current_block.clear();
                start_line = i;
            }
    ...
    fn create_code_metadata(&self, content: &str, start_line: usize, language: &str, block_type: &str) -> HashMap<String, String> {
            ...
            for (i, line) in lines.iter().enumerate() {
                ...
                if current_chunk.join("\n").len() > config.max_chunk_size {
                    ...
                    chunks.push(Chunk {
                        ...
                        end_index: i,
                        chunk_type: ChunkType::Code,
                        metadata: self.create_code_metadata(&chunk_content, start_line, &language, "mixed"),
                    });
                    current_chunk.clear();
                    start_line = i;
                }
