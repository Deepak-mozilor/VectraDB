use vectradb_components::{VectorDatabase, VectorDocument, VectorMetadata, VectraDBError, DatabaseStats};
use vectradb_search::{AdvancedSearch, HNSWIndex, LSHIndex, PQIndex, SearchAlgorithm, SearchConfig};
use ndarray::Array1;
use sled::{Db, Tree};
...
impl PersistentVectorDB {
    ...
    pub async fn new(config: DatabaseConfig) -> Result<Self, VectraDBError> {
        ...
        let index: Box<dyn AdvancedSearch + Send + Sync> = match config.search_algorithm {
            ...
            _ => return Err(VectraDBError::DatabaseError(anyhow::anyhow!("Unsupported search algorithm"))),
        };
    ...
    async fn rebuild_index(&mut self) -> Result<(), VectraDBError> {
        ...
        for result in self.vectors_tree.iter() {
            let (id_bytes, vector_bytes) = result
                .map_err(|e| VectraDBError::DatabaseError(anyhow::anyhow!(e)))?;
            ...
            // Get metadata
            let metadata_bytes = self.metadata_tree.get(&id)
                .map_err(|e| VectraDBError::DatabaseError(anyhow::anyhow!(e)))?
    ...
    async fn load_vector(&self, id: &str) -> Result<VectorDocument, VectraDBError> {
        // Load metadata
        let metadata_bytes = self.metadata_tree.get(id.as_bytes())
            .map_err(|e| VectraDBError::DatabaseError(anyhow::anyhow!(e)))?
        ...
        // Load vector data
        let vector_bytes = self.vectors_tree.get(id.as_bytes())
            .map_err(|e| VectraDBError::DatabaseError(anyhow::anyhow!(e)))?
            .ok_or_else(|| VectraDBError::VectorNotFound { id: id.to_string() })?;
...
impl VectorDatabase for PersistentVectorDB {
    fn create_vector(&mut self, id: String, vector: Array1<f32>, tags: Option<HashMap<String, String>>) -> Result<(), VectraDBError> {
        ...
        if self.vectors_tree.contains_key(id.as_bytes())
            .map_err(|e| VectraDBError::DatabaseError(anyhow::anyhow!(e)))? {
            return Err(VectraDBError::VectorNotFound { id });
        }

        let document = vectradb_components::vector_operations::create_vector_document(id.clone(), vector, tags)?;
    ...
    fn update_vector(&mut self, id: &str, vector: Array1<f32>, tags: Option<HashMap<String, String>>) -> Result<(), VectraDBError> {
        ...
        // Update document
        let updated_doc = vectradb_components::vector_operations::update_vector_document(existing_doc, vector, tags)?;
    ...
    fn search_similar(&self, query_vector: Array1<f32>, top_k: usize) -> Result<Vec<vectradb_components::SimilarityResult>, VectraDBError> {
