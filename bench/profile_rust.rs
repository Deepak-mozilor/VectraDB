// Standalone Rust profiling binary for VectraDB HNSW search.
// Build and run with: cargo build --release --example profile_rust
// Profile with: cargo flamegraph --example profile_rust
...
fn main() {
    ...
    // Build index
    println!("Building HNSW index ({n} vectors, dim={dim})...");
    ...
    // Profile: 10000 searches
    println!("Running {n_queries} searches...");
    let t0 = Instant::now();
    for q in &queries {
        let _ = index.search(q, 10);
    }
    ...
    println!("Done: {qps:.0f} QPS, avg {avg_us:.1f}us per query");
}
